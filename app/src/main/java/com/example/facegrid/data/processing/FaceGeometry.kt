...
fun isClearlyVisible(face: FaceObservation, frameWidth: Int, frameHeight: Int): Boolean {
    val areaRatio = face.boundingBox.width()
        .toFloat() * face.boundingBox.height() / (frameWidth * frameHeight).toFloat()
...
fun normalizedAverage(current: FloatArray, sample: FloatArray, count: Int): FloatArray {
    ...
    for (index in current.indices) result[index] =
        (current[index] * count + sample[index]) / (count + 1)
    var magnitude = 0.0
    for (value in result) magnitude += value * value
    val scale = if (magnitude == 0.0) 1f else (1.0 / sqrt(magnitude)).toFloat()
