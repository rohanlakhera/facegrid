...
class VideoProcessor(private val context: Context) {
    suspend fun process(uri: Uri, onProgress: (ProgressUpdate) -> Unit): ProcessingResult =
        withContext(Dispatchers.Default) {
            ...
            try {
                ...
                val totalFrames =
                    maxOf(1, ((durationMs * 1_000L + intervalUs - 1) / intervalUs).toInt())
                ...
                for (frameIndex in 0 until totalFrames) {
                    ...
                    onProgress(
                        ProgressUpdate(
                            ...
                            frameIndex + 1,
                            totalFrames,
                            "Reading frame ${frameIndex + 1}/$totalFrames"
                        )
                    )
                    ...
                    onProgress(
                        ProgressUpdate(
                            ...
                            frameIndex + 1,
                            totalFrames,
                            "Detecting faces: frame ${frameIndex + 1}/$totalFrames"
                        )
                    )
                    val rawDetections = detectFaces(detector, bitmap, frameIndex, timestampUs)
                    val detected = deduplicateDetections(rawDetections)
                    val trackable =
                        detected.filter { isTrackableFace(it, bitmap.width, bitmap.height) }
                    val visible =
                        trackable.filter { isClearlyVisible(it, bitmap.width, bitmap.height) }
                    ...
                    val unmatchedTracks =
                        tracks.filter { track -> assignments.none { it.first === track } }
                ...
                val embedded = usableSegments.mapIndexed { index, segment ->
                    ...
                    onProgress(
                        ProgressUpdate(
                            ...
                            index + 1,
                            usableSegments.size,
                            "Embedding segment ${index + 1}/${usableSegments.size}"
                        )
                    )
    ...
    private fun deduplicateDetections(faces: List<FaceObservation>): List<FaceObservation> {
        ...
        faces.sortedWith(
            ...
            val duplicate = kept.any { existing ->
                ...
                ) >= ProcessingConfig.DUPLICATE_FACE_IOU || (
                        intersectionOverSmallerArea(
                            candidate.boundingBox,
                            existing.boundingBox
                        ) >= ProcessingConfig.DUPLICATE_FACE_CONTAINMENT
                        )
            }
    ...
    private fun findAssignments(
        tracks: List<OpenTrack>,
        faces: List<FaceObservation>
    ): List<Pair<OpenTrack, FaceObservation>> {
        val candidates = tracks.flatMap { track ->
            faces.map { face ->
                Triple(
                    track,
                    face,
                    intersectionOverUnion(track.lastFrame.boundingBox, face.boundingBox)
                )
            }
    ...
    private fun closeTracks(
        tracks: List<OpenTrack>,
        segments: MutableList<AppearanceSegment>,
        nextSegmentId: Int
    ) {
